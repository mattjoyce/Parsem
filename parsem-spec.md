...
| Rating dot (right-gutter vertical stack of 5) | Click hollow → `POST /rate` sets that rating. Click filled → `POST /unrate` clears the rating (logs a `rate_clear` event). Keyboard 1–5 still sets via /rate; clearing is click-only. No state mutation beyond rate / rate_clear, no `current_position` change. | Free    | claude-axx.3 (live)          |
...
The **right gutter** (~28px) carries the rating prompt — a vertical stack of five small dots rendered alongside the bottom edge of the current chunk. The dot at the chunk's current rating (if any) is filled; the others are hollow rings. Clicking a hollow dot sets that rating; clicking the filled dot clears it (`POST /unrate`, logs a `rate_clear` event). Keyboard 1–5 still works for "set" (§7.4); clearing is click-only. The prompt is muted at rest (~40% opacity), brightens on hover, and tracks the current chunk wherever it goes (frontier, click-back, conceal-back). The earlier `1·2·3·4·5` inline-below-chunk placement was moved into the gutter on UAT feedback (claude-axx.3): the inline form interrupted the reading flow when the cursor was mid-document, and digits gave no visual feedback for a click. The right gutter remains reserved for further expansion (notes, chunk Q&A); rating is the first occupant.
